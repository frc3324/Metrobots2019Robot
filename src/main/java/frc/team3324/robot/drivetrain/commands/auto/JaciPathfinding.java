...
public class JaciPathfinding extends Command {
    ...
    protected void initialize() {
        ...
        TankModifier modifier =
            new TankModifier(trajectory).modify(Constants.DriveTrain.DISTANCE_BETWEEN_WHEELS);
    ...
    private void follow() {
        ...
        double gyroHeading =
            Robot.driveTrain.getYaw(); // Assuming the gyro is giving a value in degrees
    ...
    private void followReversed() {
        ...
        double rOutput = right.calculate(-Robot.driveTrain.rEncoder.getRaw());
        double gyroHeading =
            Robot.driveTrain.getYaw() + 180; // Assuming the gyro is giving a value in degrees
        double desiredHeading = Pathfinder.r2d(left.getHeading()); // Should also be in degrees
        angleDifference = Pathfinder.boundHalfDegrees(desiredHeading - gyroHeading);
    ...
    private void updateShuffleBoard(double lOutput, double rOutput, double gyroHeading,
    ...
    protected boolean isFinished() {
        return (left.isFinished() && right.isFinished() && Math.abs(angleDifference) < 3) ||
            OI.primaryController.getBackButton();
    }
}
